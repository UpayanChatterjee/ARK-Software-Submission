...
CAMERA_MATRIX = np.array([6.9542619962191930e+02, 0., 3.3595878994582023e+02, 0.,
                          7.0123291039058915e+02, 2.0328731927191703e+02, 0., 0., 1.])

CAMERA_MATRIX = np.float32(CAMERA_MATRIX.reshape(3, 3))

DIST_COEFF = np.array([2.2201115943292554e-01, -9.0652859085691029e-01,
...
def determine_pose(frame, aruco_list, aruco_dict):
    axes = np.array([
        [0.0, 0.0, 0.0],
        [10.0, 0.0, 0.0],
        [0.0, 10.0, 0.0],
    ...
    for aruco in aruco_list:
        retval, rvec, tvec = cv.solvePnP(
            np.float32(aruco_dict.world_coord[aruco.ids]), np.float32(aruco.corners), CAMERA_MATRIX, DIST_COEFF)

        imgpoints, _ = cv.projectPoints(
            axes, rvec, tvec, CAMERA_MATRIX, DIST_COEFF)
        imgpoints = np.int32(imgpoints)
        # print(imgpoints)
        cv.line(frame, (int(imgpoints[0][0][0]), int(imgpoints[0][0][1])),
                (imgpoints[1][0][0], imgpoints[1][0][1]), (0, 0, 255), 2, cv.LINE_AA)
        cv.line(frame, (imgpoints[0][0][0], imgpoints[0][0][1]),
                (imgpoints[2][0][0], imgpoints[2][0][1]), (0, 255, 0), 2, cv.LINE_AA)
        cv.line(frame, (imgpoints[0][0][0], imgpoints[0][0][1]),
                (imgpoints[3][0][0], imgpoints[3][0][1]), (255, 0, 0), 2, cv.LINE_AA)
...
def main():
    ...
    while True:
        ...
        determine_pose(frame, result, aruco_dict)
